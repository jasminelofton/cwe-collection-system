...
# ret: ALLOWED or PROHIBITTED or RESTRICTED.
def get_vulnerability_mapping(soup):
    #return soup.find('html', recursive=False).find('body', recursive=False).find('table', recursive = False).find('span', class_='tool').find('span').get_text(strip=True)
    return soup.select_one('body table span.tool span').get_text(strip=True)
